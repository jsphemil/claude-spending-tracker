...
- **Dashboard/cash-flow redesign.** Replaced the ring visual and the separate `/summary` page entirely — the Dashboard is now the single home for everything, and is itself month-navigable:
    - **Net worth** (as of end of the selected month) = sum of every account's Ending Balance for that month — credit card balances are negative and pull it down naturally (₹2L in FD + ₹1.5L credit card due = ₹50K net worth, not ₹2L), with **Credit card debt** broken out as its own line
    - **Carry Forward** (prior month's Ending Balance), **Total In** (Income + Transfer In), **Total Out** (Expense + Transfer Out), **Left to Spend** (Total In − Total Out) — shown at both the all-accounts (Dashboard) and single-account level, no Budget Mode required
    - A 4-segment cash-flow **pie chart** (Recharts) per period: Income (green), Transfer In (blue), Expense (red), Transfer Out (yellow), sized by actual amount — replaces the old progress-ring entirely
    - Income-by-category and Expense-by-category pie charts, unchanged in spirit from the original Summary page
    - Each account's page mirrors this same panel, scoped to just that account, plus a simple credit-utilization bar (％ of credit limit used) for credit cards
