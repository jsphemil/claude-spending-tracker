...
- **Currency list expanded to full FX-provider support.** Account currency was a curated 9-option list (including AED, which isn't an ECB reference currency). Replaced with the full ~31-currency set Phase 8's planned provider (Frankfurter, ECB reference rates) actually supports, so any currency selectable at account creation is guaranteed convertible once Phase 8 ships — picking a currency can no longer promise conversion it can't deliver. `COMMON_CURRENCIES` (bare codes) renamed to `CURRENCIES` ({code, name} objects, alphabetical by code) in `lib/constants/accounts.ts`; dropdown now shows "USD — US Dollar" instead of a bare code.

### Remaining

- **Phase 8 — Multi-currency.** Live FX rates (Frankfurter API) with caching, `CurrencyAmount` display component, true net-worth "All Accounts" total across currencies. Account currency selection is now ready for this (see above).
