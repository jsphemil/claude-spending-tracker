...
- **Roadmap #4 — Spend-pace / month-end projection.** Account detail page (current month, non-credit only) now shows "At this rate, by month-end: ₹X," extrapolated from what's actually gone out *so far* (deliberately excludes future-dated transactions already entered this month, e.g. a recurring bill scheduled for the 25th when today is the 10th — those haven't happened yet and would inflate the daily average) divided by days elapsed, times days in the month. Turns red with a "That's ₹Y more than you have available" note when the projection exceeds this period's available funds. Verified live: 15,700 spent over 10 elapsed days × 31 days = 48,670 projected, 23,170 over the 25,500 available — correct, though a naive linear projection like this will always be skewed by one big early purchase (inherent to the method, not a bug; every budgeting app that does this has the same property).
...
| ~~4~~ | ~~Spend-pace / month-end projection~~ — **Shipped** | 8 | 4 | **+4** |
