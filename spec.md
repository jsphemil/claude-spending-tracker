...
- **Desktop-grid layout for remaining pages.** Only the Dashboard got the new multi-column grid treatment so far — Accounts, Transactions, Categories, Goals, Commitments, Account detail, and all forms still use their original single-column layout (now correctly colored for both themes, just not regridded). Natural next step if the user wants the full app to match, not yet requested beyond Dashboard.
